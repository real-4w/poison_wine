def assign_bottles_to_wine_makers(num_bottles):
    ...
    Returns a list where each element is a set of bottle numbers for a wine_maker.
    """
    num_wine_makers = 10
    ...
    for bottle in range(num_bottles):
        ...
        for wine_maker in range(num_wine_makers):
            # Bit 0 (LSB) -> wine_maker 1, Bit 9 (MSB) -> wine_maker 10
            if binary[9 - wine_maker] == '1':  # Read from right (LSB) to left (MSB)
                wine_makers[wine_maker].add(bottle)
...
def simulate_poisoning(wine_makers, poisoned_bottle):
    ...
    for wine_maker in range(num_wine_makers):
        if poisoned_bottle in wine_makers[wine_maker]:
            sick_wine_makers[wine_maker] = 1
...
def identify_poisoned_bottle(sick_wine_makers):
    ...
    """
    # Convert sick_wine_makers list to binary string (wine_maker 1 is LSB, wine_maker 10 is MSB)
...
def run_simulation(num_bottles, poisoned_bottle):
    ...
    for i, wine_maker_set in enumerate(wine_makers, 1):
        if bottle_to_check in wine_maker_set:
            print(f"wine_maker {i} drinks from bottle {bottle_to_check}")
...
num_bottles = 1000
test_cases = [0,1,2,10,100,999]  # Example poisoned bottles
